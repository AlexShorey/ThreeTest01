...
class TargetReticle {
    ...
    getCSS3D_cameraStyle(cam: THREE.PerspectiveCamera, fov: number): string {
        var cssStyle = "";
...
*/
function camToCSSFov(f: number): number {
    var fov = f;
    fov = 0.5 / Math.tan(fov * Math.PI / 360) * window.innerHeight;
    return fov;
}

function toCSSMatrix(threeMat4: THREE.Matrix4, b: bool): string {
    var a = threeMat4, f;
    if (b) {
        f = [
